...
#add layers
model.add(tf.keras.layers.Conv2D(64, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg, input_shape=(128,128,1)))#kernel_regularizer=reg, input_shape=(128,128,1)))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.Activation('relu'))
model.add(tf.keras.layers.Conv2D(64, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg))#,  kernel_regularizer=reg) )
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.Activation('relu'))
model.add(tf.keras.layers.Conv2D(64, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg))#,  kernel_regularizer=reg) )
model.add(tf.keras.layers.BatchNormalization())
...
model.add(tf.keras.layers.Conv2D(128, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg))#,  kernel_regularizer=reg))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.Activation('relu'))
model.add(tf.keras.layers.Conv2D(128, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg))#, kernel_regularizer=reg))
model.add(tf.keras.layers.BatchNormalization())
...
model.add(tf.keras.layers.SpatialDropout2D(0.2))
model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))
model.add(tf.keras.layers.Conv2D(256, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg))#, kernel_regularizer=reg))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.Activation('relu'))
model.add(tf.keras.layers.Conv2D(256, kernel_size=(3,3), padding = 'same', kernel_regularizer=reg))#, kernel_regularizer=reg))
model.add(tf.keras.layers.BatchNormalization())
...
twotwothreeSGD = model.fit(X_train, y_train, validation_data=(X_valid, y_valid), epochs=100, use_multiprocessing=True)

print('Saving history')

f = open('twotwothreesgdnestl2.pickle', 'wb')
